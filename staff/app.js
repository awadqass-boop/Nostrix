...
const S={db:null,user:null,profile:null,profiles:[],projects:[],tasks:[],comments:[],links:[],files:[],activity:[],notifications:[],view:"mine",search:"",priority:"all",assignee:"all",project:"all",taskId:null,lang:localStorage.getItem("nostrix_language_v1")==="ar"?"ar":"en",rt:null,announcements:[],quickLinks:[],calendarMonth:new Date(new Date().getFullYear(),new Date().getMonth(),1),calendarSelected:new Date(),dateViews:{}};
...
async function loadAll(){await materializeRecurring();await Promise.all([loadProfiles(),loadProjects(),loadTasks(),loadNotifs(),loadAnnouncements(),loadQuickLinks()]);renderAll()}
...
async function loadNotifs(){if(!S.user)return;let{data,error}=await S.db.from("notifications").select("*").eq("user_id",S.user.id).order("created_at",{ascending:false}).limit(40);if(error)console.error(error);S.notifications=data||[]}
async function loadAnnouncements(){let{data,error}=await S.db.from("announcements").select("*").order("created_at",{ascending:false});if(error)console.error(error);S.announcements=(data||[]).filter(a=>a.active!==false&&(!a.expires_on||days(a.expires_on)>=0))}
async function loadQuickLinks(){let{data,error}=await S.db.from("quick_links").select("*").order("sort_order").order("created_at");if(error)console.error(error);S.quickLinks=(data||[]).filter(q=>q.active!==false)}
...
function renderAll(){if(!$("#app-screen")||$("#app-screen").hidden)return;fillOptions();renderAnnouncements();renderQuickLinks();renderProjects();renderTasks();renderNotifs();renderCalendar();renderAdminAccess()}

function renderAnnouncements(){let e=$("#announcement-list");if(!e)return;e.innerHTML=S.announcements.length?S.announcements.slice(0,4).map(a=>`<article class="announcement-item level-${esc(a.level||"info")}"><span class="announcement-icon"><i class="ph ${a.level==="urgent"?"ph-warning-octagon":a.level==="important"?"ph-star":"ph-megaphone"}"></i></span><div><div class="announcement-title-row"><strong>${esc(a.title)}</strong><small>${esc(dt(a.created_at))}</small></div><p>${esc(a.body)}</p></div></article>`).join(""):`<div class="widget-empty"><i class="ph ph-megaphone-simple"></i><span>No announcements right now.</span></div>`}
function renderQuickLinks(){let e=$("#quick-links");if(!e)return;e.innerHTML=S.quickLinks.length?S.quickLinks.slice(0,8).map(q=>`<a class="quick-link" href="${esc(q.url)}" target="_blank" rel="noopener"><span><i class="ph ${iconForQuickLink(q.icon)}"></i></span><div><strong>${esc(q.label)}</strong><small>Open link</small></div><i class="ph ph-arrow-up-right"></i></a>`).join(""):`<div class="widget-empty"><i class="ph ph-link-simple"></i><span>No quick links yet.</span></div>`}
function renderAdminAccess(){let b=$("#admin-btn");if(b)b.hidden=!isAdmin()}
function calendarEvents(){return [...S.tasks.filter(t=>t.due_date).map(t=>({date:t.due_date,type:"task",id:t.id,title:t.title,meta:taskAssigneeName(t),priority:t.priority})),...S.projects.filter(p=>p.due_date&&p.status==="active").map(p=>({date:p.due_date,type:"project",id:p.id,title:p.name,meta:p.client_name||"Project",priority:"project"}))]}
function renderCalendar(){let g=$("#calendar-grid");if(!g)return;let m=S.calendarMonth,y=m.getFullYear(),mo=m.getMonth(),first=new Date(y,mo,1),start=new Date(y,mo,1-first.getDay()),events=calendarEvents();$("#calendar-month-label").textContent=monthName(m);let h="";for(let i=0;i<42;i++){let d=new Date(start);d.setDate(start.getDate()+i);let k=ymd(d),ev=events.filter(x=>x.date===k);h+=`<button class="calendar-day ${d.getMonth()===mo?"":"outside"} ${k===ymd(new Date())?"today":""} ${k===ymd(S.calendarSelected)?"selected":""}" type="button" data-cal-date="${k}"><span>${d.getDate()}</span>${ev.length?`<div class="day-events">${ev.slice(0,3).map(x=>`<i class="${x.type} ${x.priority||""}"></i>`).join("")}${ev.length>3?`<em>+${ev.length-3}</em>`:""}</div>`:""}</button>`}g.innerHTML=h;$$("[data-cal-date]",g).forEach(b=>b.onclick=()=>{S.calendarSelected=parseYmd(b.dataset.calDate);renderCalendar()});renderAgenda()}
function renderAgenda(){let k=ymd(S.calendarSelected),e=calendarEvents().filter(x=>x.date===k),l=$("#agenda-list");$("#agenda-date-label").textContent=prettyDate(k);l.innerHTML=e.length?e.map(x=>`<button class="agenda-item ${x.type}" type="button" data-agenda-type="${x.type}" data-agenda-id="${x.id}"><span class="agenda-dot"></span><div><strong>${esc(x.title)}</strong><small>${esc(x.meta)}</small></div><i class="ph ph-caret-right"></i></button>`).join(""):`<div class="agenda-empty"><i class="ph ph-calendar-check"></i><span>Nothing scheduled for this date.</span></div>`;$$("[data-agenda-type]",l).forEach(b=>b.onclick=()=>{if(b.dataset.agendaType==="task")openTask(b.dataset.agendaId);else{S.project=b.dataset.agendaId;$("#project-filter").value=S.project;S.view="all";setView("all");renderTasks()}})}
...
function clearProject(){$("#project-id").value="";$("#project-name").value="";$("#project-client").value="";setDateValue("project-due","",false);$("#project-status").value="active";refreshCustomSelects()}
...
function editProject(id){let p=pr(id);$("#project-id").value=id;$("#project-name").value=p.name;$("#project-client").value=p.client_name||"";setDateValue("project-due",p.due_date||"",false);$("#project-status").value=p.status;refreshCustomSelects()}
...
function openAdmin(){if(!isAdmin())return toast("Admin access required");$("#profile-menu").hidden=true;$("#admin-modal").hidden=false;document.body.style.overflow="hidden";renderAdminLists()}
function closeAdmin(){$("#admin-modal").hidden=true;document.body.style.overflow="";clearAnnouncement();clearQuickLink()}
function switchAdminTab(n){$$(".admin-tab").forEach(b=>b.classList.toggle("active",b.dataset.adminTab===n));$$(".admin-panel").forEach(p=>{let a=p.dataset.adminPanel===n;p.hidden=!a;p.classList.toggle("active",a)})}
function clearAnnouncement(){$("#announcement-id").value="";$("#announcement-title").value="";$("#announcement-body").value="";$("#announcement-level").value="info";setDateValue("announcement-expiry","",false);refreshCustomSelects()}
async function saveAnnouncement(e){e.preventDefault();let id=$("#announcement-id").value,p={title:$("#announcement-title").value.trim(),body:$("#announcement-body").value.trim(),level:$("#announcement-level").value,expires_on:$("#announcement-expiry").value||null,active:true};let r=id?await S.db.from("announcements").update(p).eq("id",id):await S.db.from("announcements").insert({...p,created_by:S.user.id});if(r.error)return toast(r.error.message);clearAnnouncement();await loadAnnouncements();renderAnnouncements();renderAdminLists();toast("Announcement saved")}
function editAnnouncement(id){let a=S.announcements.find(x=>x.id===id);if(!a)return;$("#announcement-id").value=id;$("#announcement-title").value=a.title;$("#announcement-body").value=a.body;$("#announcement-level").value=a.level||"info";setDateValue("announcement-expiry",a.expires_on||"",false);refreshCustomSelects()}
async function deleteAnnouncement(id){if(!confirm("Delete this announcement?"))return;let{error}=await S.db.from("announcements").delete().eq("id",id);if(error)return toast(error.message);await loadAnnouncements();renderAnnouncements();renderAdminLists()}
function clearQuickLink(){$("#quicklink-id").value="";$("#quicklink-label").value="";$("#quicklink-url").value="";$("#quicklink-icon").value="folder";$("#quicklink-order").value="0";refreshCustomSelects()}
async function saveQuickLink(e){e.preventDefault();let id=$("#quicklink-id").value,p={label:$("#quicklink-label").value.trim(),url:$("#quicklink-url").value.trim(),icon:$("#quicklink-icon").value,sort_order:Number($("#quicklink-order").value)||0,active:true};let r=id?await S.db.from("quick_links").update(p).eq("id",id):await S.db.from("quick_links").insert({...p,created_by:S.user.id});if(r.error)return toast(r.error.message);clearQuickLink();await loadQuickLinks();renderQuickLinks();renderAdminLists();toast("Quick link saved")}
function editQuickLink(id){let q=S.quickLinks.find(x=>x.id===id);if(!q)return;$("#quicklink-id").value=id;$("#quicklink-label").value=q.label;$("#quicklink-url").value=q.url;$("#quicklink-icon").value=q.icon||"folder";$("#quicklink-order").value=q.sort_order||0;refreshCustomSelects()}
async function deleteQuickLink(id){if(!confirm("Delete this quick link?"))return;let{error}=await S.db.from("quick_links").delete().eq("id",id);if(error)return toast(error.message);await loadQuickLinks();renderQuickLinks();renderAdminLists()}
async function toggleAdmin(uid,next){let{error}=await S.db.from("profiles").update({is_admin:next}).eq("user_id",uid);if(error)return toast(error.message);await loadProfiles();S.profile=pf(S.user.id)||S.profile;renderAdminAccess();renderAdminLists();toast("Team role updated")}
function renderAdminLists(){if(!isAdmin())return;$("#admin-announcements-list").innerHTML=S.announcements.length?S.announcements.map(a=>`<article class="admin-row"><div><strong>${esc(a.title)}</strong><small>${esc(a.level)}${a.expires_on?" · "+prettyDate(a.expires_on):""}</small></div><div><button class="text-btn" data-ea="${a.id}">Edit</button><button class="text-btn admin-delete" data-da="${a.id}">Delete</button></div></article>`).join(""):`<div class="empty">No announcements yet.</div>`;$("#admin-links-list").innerHTML=S.quickLinks.length?S.quickLinks.map(q=>`<article class="admin-row"><div><strong>${esc(q.label)}</strong><small>${esc(q.url)}</small></div><div><button class="text-btn" data-eq="${q.id}">Edit</button><button class="text-btn admin-delete" data-dq="${q.id}">Delete</button></div></article>`).join(""):`<div class="empty">No quick links yet.</div>`;$("#team-role-list").innerHTML=S.profiles.map(p=>`<article class="admin-row team-role-row"><div><span class="mini-avatar">${initials(p.display_name||p.email)}</span><span><strong>${esc(p.display_name||p.email)}</strong><small>${esc(p.job_title||p.email)}</small></span></div><button class="role-toggle ${p.is_admin?"active":""}" type="button" data-role-user="${p.user_id}" data-role-next="${p.is_admin?"false":"true"}"><i class="ph ${p.is_admin?"ph-shield-check":"ph-user"}"></i>${p.is_admin?"Admin":"Staff"}</button></article>`).join("");$$("[data-ea]").forEach(b=>b.onclick=()=>editAnnouncement(b.dataset.ea));$$("[data-da]").forEach(b=>b.onclick=()=>deleteAnnouncement(b.dataset.da));$$("[data-eq]").forEach(b=>b.onclick=()=>editQuickLink(b.dataset.eq));$$("[data-dq]").forEach(b=>b.onclick=()=>deleteQuickLink(b.dataset.dq));$$("[data-role-user]").forEach(b=>b.onclick=()=>toggleAdmin(b.dataset.roleUser,b.dataset.roleNext==="true"))}
...
function uiUser(){let n=S.profile?.display_name||S.user.email.split("@")[0];$("#user-name").textContent=n;$("#user-email").textContent=S.user.email;$("#hello-name").textContent=n.split(" ")[0];$("#avatar").textContent=initials(n);renderAdminAccess()}
...
async function subscribe(){if(S.rt)await S.db.removeChannel(S.rt);S.rt=S.db.channel("nostrix-v8").on("postgres_changes",{event:"*",schema:"public",table:"tasks"},async()=>{await loadTasks();renderAll()}).on("postgres_changes",{event:"*",schema:"public",table:"projects"},async()=>{await loadProjects();renderAll()}).on("postgres_changes",{event:"*",schema:"public",table:"announcements"},async()=>{await loadAnnouncements();renderAnnouncements();if(isAdmin())renderAdminLists()}).on("postgres_changes",{event:"*",schema:"public",table:"quick_links"},async()=>{await loadQuickLinks();renderQuickLinks();if(isAdmin())renderAdminLists()}).on("postgres_changes",{event:"INSERT",schema:"public",table:"notifications",filter:`user_id=eq.${S.user.id}`},async p=>{playQuack();await loadNotifs();renderNotifs();if("Notification"in window&&Notification.permission==="granted"&&document.hidden)new Notification(p.new.title,{body:p.new.body||"",icon:"/favicon.png"})}).subscribe()}
async function signout(){if(S.rt)await S.db.removeChannel(S.rt);await S.db.auth.signOut();show("auth-screen")}
function bind(){$$(".lang").forEach(b=>b.onclick=()=>{S.lang=S.lang==="ar"?"en":"ar";applyLang()});$("#login-form").onsubmit=login;initDatePickers();$("#home-btn").onclick=()=>window.scrollTo({top:0,behavior:"smooth"});$("#assignee-trigger").onclick=e=>{e.stopPropagation();$("#assignee-menu").hidden?openAssigneeMenu():closeAssigneeMenu()};document.addEventListener("click",e=>{if(!e.target.closest("#assignee-picker"))closeAssigneeMenu();if(!e.target.closest(".nostrix-select"))closeAllCustomSelects();if(!e.target.closest(".date-picker"))closeDatePickers()});document.addEventListener("keydown",e=>{if(e.key==="Escape"){closeAssigneeMenu();closeAllCustomSelects()}});refreshCustomSelects();$("#new-task").onclick=()=>openTask();$("#task-form").onsubmit=saveTask;$("#delete-task").onclick=delTask;$$("[data-close-task]").forEach(b=>b.onclick=closeTask);$$(".task-tab").forEach(b=>b.onclick=()=>switchTab(b.dataset.tab));$("#comment-form").onsubmit=comment;$("#link-form").onsubmit=addLink;$("#file-input").onchange=e=>{if(e.target.files[0])upload(e.target.files[0]);e.target.value=""};$("#projects-btn").onclick=$("#manage-projects").onclick=openProjects;$("#admin-btn").onclick=openAdmin;$$("[data-close-admin]").forEach(b=>b.onclick=closeAdmin);$$(".admin-tab").forEach(b=>b.onclick=()=>switchAdminTab(b.dataset.adminTab));$("#announcement-form").onsubmit=saveAnnouncement;$("#announcement-clear").onclick=clearAnnouncement;$("#quicklink-form").onsubmit=saveQuickLink;$("#quicklink-clear").onclick=clearQuickLink;$("#cal-prev").onclick=()=>{S.calendarMonth=new Date(S.calendarMonth.getFullYear(),S.calendarMonth.getMonth()-1,1);renderCalendar()};$("#cal-next").onclick=()=>{S.calendarMonth=new Date(S.calendarMonth.getFullYear(),S.calendarMonth.getMonth()+1,1);renderCalendar()};$("#cal-today").onclick=()=>{S.calendarSelected=new Date();S.calendarMonth=new Date(new Date().getFullYear(),new Date().getMonth(),1);renderCalendar()};$$("[data-close-projects]").forEach(b=>b.onclick=closeProjects);$("#project-form").onsubmit=saveProject;$("#project-clear").onclick=clearProject;$("#edit-profile").onclick=()=>{$("#profile-menu").hidden=true;openProfile()};$$("[data-close-profile]").forEach(b=>b.onclick=()=>{$("#profile-modal").hidden=true;document.body.style.overflow=""});$("#profile-form").onsubmit=saveProfile;$("#signout").onclick=signout;$("#refresh-btn").onclick=async()=>{await loadAll();toast("Refreshed")};$("#profile-btn").onclick=()=>$("#profile-menu").hidden=!$("#profile-menu").hidden;$("#notif-btn").onclick=()=>$("#notif-panel").hidden=!$("#notif-panel").hidden;$("#mark-read").onclick=markRead;$("#enable-browser").onclick=browserAlerts;$$(".view").forEach(b=>b.onclick=()=>{setView(b.dataset.view);renderTasks()});$("#search").oninput=e=>{S.search=e.target.value.trim();renderTasks()};$("#priority-filter").onchange=e=>{S.priority=e.target.value;renderTasks()};$("#assignee-filter").onchange=e=>{S.assignee=e.target.value;renderTasks()};$("#project-filter").onchange=e=>{S.project=e.target.value;renderTasks()};$$(".column").forEach(c=>{c.ondragover=e=>e.preventDefault();c.ondrop=e=>{e.preventDefault();move(e.dataTransfer.getData("text/plain"),c.dataset.status)}})}
